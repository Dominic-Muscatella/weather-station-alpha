...
The humidity turned out to be the stubborn part. rtl_433's lossy text dump kept desyncing when I tried to re-decode the Manchester bits by hand, so in the end I patched the C decoder to spit out every decoded byte as debug json, rebuilt the library, and ran a controlled humidity swing at a near-constant room temperature, noting the sensor's own LCD reading at nine points. That isolated it: the humidity is packed as BCD split across two non-adjacent bytes, with the tens digit in the low nibble of one byte and the ones digit in the high nibble of an earlier byte. The original freezer decoder only ever used one bit of that earlier byte, for the temperature sign, and threw the rest away. Verified 9 for 9 against the LCD from 54 to 89 percent.
